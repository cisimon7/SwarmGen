...
class multi_agent_qp_node_base_init():
	...
	@partial(jit, static_argnums=(0,))
	def solve(self, primal_sol_x, primal_sol_y, primal_sol_z, lamda_x, lamda_y, lamda_z, state_x, state_y, state_z, x_ell, y_ell, z_ell, a_ell, b_ell, c_ell):
	
		
		b_eq_x, b_eq_y, b_eq_z = self.compute_boundary_vec_batch(state_x, state_y, state_z)

		alpha_agent, beta_agent, d_agent, lamda_x, lamda_y, lamda_z, primal_residual, alpha_v, beta_v, d_v, alpha_a, beta_a, d_a, alpha_ell, beta_ell, d_ell = self.compute_alpha_d(
			primal_sol_x, primal_sol_y, primal_sol_z, lamda_x, lamda_y, lamda_z, x_ell, y_ell, z_ell, a_ell, b_ell, c_ell
		)
		...
		primal_sol_x_init = primal_sol_x# jnp.zeros(( self.num_batch, self.nvar*self.num_agent   ))
		primal_sol_y_init = primal_sol_y# jnp.zeros(( self.num_batch, self.nvar*self.num_agent   ))
		primal_sol_z_init = primal_sol_z# jnp.zeros(( self.num_batch, self.nvar*self.num_agent   ))
