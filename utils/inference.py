...
def run(
        ...
        num_agent, num=50, batch_size=10, lheight=3, lwidth=3, device="cuda", verbose=True, comp_type="base"
    ):
    x_init, y_init, z_init, x_fin, y_fin, z_fin, center, radii = (
    ...
    with th.no_grad():
        ...
        if verbose:
            print(f"PixelCNN sampling time: {(perf_counter() - start):.5f} seconds")
        ...
        if verbose:
            print(f"VQ decoder time: {(perf_counter() - start):.5f} seconds")
        ...
        if verbose:
            print(f"Learned optimizer time: {(perf_counter() - start):.5f} seconds")
        ...
        if verbose:
            print(f"Base/Comp optimizer time: {(perf_counter() - start):.5f} seconds")
