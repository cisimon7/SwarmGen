...
def get_coeffs(num, t_fin, device):
    tot_time = np.linspace(0, t_fin, num)
    tot_time_copy = tot_time.reshape(num, 1)

    P, Pdot, Pddot = bernstein_coeff_order10_new(10, tot_time_copy[0], tot_time_copy[-1], tot_time_copy)
    P = th.from_numpy(P).float().to(device=device)
    Pdot = th.from_numpy(Pdot).float().to(device=device)
    Pddot = th.from_numpy(Pddot).float().to(device=device)
